...
class double_debias:
    ...
    def __init__(self, y=None, D=None, z=None, D_method=None, y_method=None, n_folds=2):
        ...
        ---
            ...
            D_method : sklearn model
                slearn model such as sklearn.ensemble.GradientBoostingRegressor. Can be any class that implements the sklearn API of 
        ...
        self.models = {'y': [copy.deepcopy(self.methods['y']) for i in range(self.KFolds.n_splits)], 'D': [copy.deepcopy(self.methods['D']) for i in range(self.KFolds.n_splits)]}
    ...
    def est_thetas(self):
        ...
        self.thetas = np.array([self.theta_formula(Y, V, D[1]) for (Y, V, D) in zip(self.residualize_('y'), self.residualize_('D'), self.KFolds_split_('D')) ])
        return self.thetas
...
ob = double_debias(y=np.array([i for i in range(0,10)]), 
